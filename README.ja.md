...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 040 作例 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 040 作例 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 040 作例 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 040 作例 8"></td>
  </tr>
</table>

## 3:4 縦長・上下構成の作例

16:9 組とは別の独立した4点で、3:4 上下の完成キャンバスを生成。上部は現実の写真を保ち、下部は Panel 040 の原文プロンプトに従って再構成しています。英語コピーは各写真の内容・空気・含意だけから生成し、全点を個別に確認しました。
...
**Panel 040** は、原写真の核を保ちながら、次の視覚方向を目指す制作に向いています：**写真が本当に語っていることを、ひとつの実在アンカーと小さな線画の物語へ**。まず用途と期待する結果を確認し、そこから構図、サイズ、文字、納品形式を決めます。
...
> XXD Panel 040 でこの画像を処理して、まず最適な構図とサイズを提案して。

視覚方向を先に確認したい場合は、上の作例をご覧ください。準備ができたら[使い始める](#使い始める)へ進めます。全モードと引数は折りたたみ資料から必要に応じて確認できます。
<!-- xxd-human-intro:end -->

<!-- xxd-panel-benefit:start -->
## すぐに判断：XXD Panel 040 はあなたに合う？

| 知りたいこと | このスタイルの答え |
|---|---|
| **得られる完成作品** | 写真が本当に語っていることを、ひとつの実在アンカーと小さな線画の物語へ |
| **ひと目で分かる特徴** | 実在する主役 · 黒い線画の小人 · ミクロな物語 · 大きな余白 |
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 040 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-040 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-040 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-040 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-040 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 040 を適用し、共通設定は一度だけ解決
/xxd-panel-040 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
