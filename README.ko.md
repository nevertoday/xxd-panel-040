...
모드를 지정하지 않으면 Skill이 먼저 묻습니다. 같은 답변에서 크기를 지정할 수 있으며 정확한 픽셀값이 가장 우선합니다.

| 모드 | 크기 규칙 | 결과물 |
| --- | --- | --- |
| `top-bottom` | 원본 적응형 | 위에는 전체 원본, 아래에는 040 미니 서사. 두 패널이 원본 크기를 유지하며 정확히 50/50 |
| `left-right` | 원본 적응형 | 왼쪽에는 전체 원본, 오른쪽에는 040 미니 서사. 두 패널이 원본 크기를 유지하며 정확히 50/50 |
| `design-only` | 원본 적응형 | 원본 사진은 보이지 않고 변환 디자인만 표시. 원본 비율과 크기 유지 |
| `wallpaper-pack` | 기기별 네 가지 크기 | 휴대전화, iPad, 데스크톱, 어린이용 스마트워치 PNG 개별 출력 |

정확한 사용자 픽셀 > 지정 비율 또는 용도 > 일반 모드의 원본 적응형 순서입니다. 원본 `040.md`의 3:4는 초기 창작 캔버스일 뿐 현재 Skill의 숨은 기본값이 아닙니다.
...
- **독립형 네 작품:** 각 기기는 원본 사진만 참조하며 앵커 위치와 인물 상호작용을 더 자유롭게 탐색합니다.

연결형은 크롭이 아닙니다. 네 장 모두 따로 생성하고 구성하며 검수하고, iPad→휴대전화→데스크톱→스마트워치 순으로 참조를 이어 붙이지 않습니다.

## 문구는 ‘아, 그렇구나’ 하는 순간을 만들어야 합니다

생성 전에 자동 문구, 사용자 지정 문구, 무문자 출력 중 하나를 선택합니다. 문구가 있으면 대상 언어나 지역도 지정합니다.

자동 문구는 눈에 보이는 사실, 관계의 긴장, 두 요소가 함께 암시하는 근거 있는 함의를 읽습니다. 그런 다음 정확한 명명, 절제된 표현, 중의성, 가벼운 유머, 작은 반전을 이용해 매우 짧은 손글씨 한마디를 만듭니다.

보이는 물체의 이름을 반복하거나 ‘꿈·기억·여행’ 같은 만능 단어에 기대지 않습니다. 무관한 사진에도 같은 힘으로 어울리는 문구는 다시 써야 합니다.
...
이미지 모델이 실제 앵커, 인물, 여백, 문구를 만듭니다. `scripts/compose_panel.py`는 캔버스 계획, 정확한 50/50 래스터 합성, 최종 크기와 검수만 수행하며 프로그램 그림으로 결과물을 위조하지 않습니다.
...
- [Skill 워크플로](SKILL.md)
- [중문 전체 프롬프트](references/xxd-panel-040-prompt.zh-CN.md)
- [영문 전체 프롬프트](references/xxd-panel-040-prompt.en.md)
...
- 일반 모드는 원본마다 한 장, 배경화면 모드는 연락처 시트가 아닌 정확히 네 개의 독립 파일을 반환합니다.
