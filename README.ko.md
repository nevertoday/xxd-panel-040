<p align="center">
  <img src="./assets/banner.svg" alt="XXD Panel 040 프로젝트 배너" width="1200">
</p>
...
### 사진이 정말 말하고 있는 것을 하나의 실제 앵커와 작은 검은 선 이야기로 바꿉니다

[![Codex Skill](https://img.shields.io/badge/Codex-Skill-000000?style=flat-square)](./SKILL.md)
[![Four Modes](https://img.shields.io/badge/Modes-4-d75d32?style=flat-square)](#네-가지-출력-하나의-서사-논리)
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 040 예시 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 040 예시 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 040 예시 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 040 예시 8"></td>
  </tr>
</table>
...
> 실제 주인공 · 검은 선 인물 · 미니 서사 · 넉넉한 여백

XXD Panel 040은 Codex와 호환 에이전트를 위한 이미지 생성 Skill입니다. 사진에 귀여운 낙서를 덧붙이는 도구가 아닙니다. 먼저 사진에서 무엇이 가장 기억될 가치가 있는지 읽고, 실제 재질과 정체성을 가진 시각적 앵커 하나를 남긴 뒤, 소수의 검은 선 인물이 원래 이야기에 반응하거나 그것을 확대하고 살짝 뒤집게 합니다.
...
**Panel 040**은 원본 사진의 핵심을 보존하면서 다음과 같은 시각 방향을 원하는 작업에 적합합니다: **사진이 정말 말하고 있는 것을 하나의 실제 앵커와 작은 검은 선 이야기로 바꿉니다**. 먼저 용도와 원하는 결과를 확인한 뒤 구성, 크기, 텍스트와 전달 형식을 결정합니다.
...
> XXD Panel 040으로 이 이미지를 작업하고, 먼저 가장 어울리는 구성과 크기를 추천해 줘.
...
## 빠른 판단: XXD Panel 040이 나에게 맞을까요?

| 궁금한 점 | 이 스타일이 주는 답 |
|---|---|
| **얻게 되는 결과** | 사진이 정말 말하고 있는 것을 하나의 실제 앵커와 작은 검은 선 이야기로 바꿉니다 |
| **한눈에 알아볼 특징** | 실제 주인공 · 검은 선 인물 · 미니 서사 · 넉넉한 여백 |
...
```text
사실 읽기 → 관계 찾기 → 함의 압축 → 실제 앵커 선택 → 상호작용 설계 → 화면 밖 한마디 쓰기
```

무관한 사진으로 바꿔도 앵커, 인물 행동, 공간 관계, 색, 문구가 거의 그대로 성립한다면 040이 아닙니다.

## 완성작에서 바로 알아볼 수 있는 스타일 특징

- **실제 앵커:** 사진적 신뢰감, 재질의 정체성, 결정적인 윤곽, 원본의 주요 색을 보존하고 주인공을 만화로 바꾸지 않습니다.
- **최소 세 가지 고유 단서:** 윤곽, 자세, 방향, 비례, 재질, 색, 구멍, 음형, 관계 거리가 원본과의 연결을 지킵니다.
- **검은 선 인물 1–5명:** 가늘고 조금 서툰 듯한 선이지만 행동은 명확하며, 모든 몸짓이 원본의 함의를 설명해야 합니다.
- **붙이기가 아닌 상호작용:** 인물이 앵커의 가장자리, 개구부, 표면, 방향, 크기를 실제 무대로 사용합니다.
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 040의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-040 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-040 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-040 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-040 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-040 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-040 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 040 적용, 공통 설정은 한 번만 확정
/xxd-panel-040 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
Claude Code 사용자는 같은 폴더를 `~/.claude/skills/xxd-panel-040`에 연결할 수 있습니다. 설치 후 에이전트 세션을 다시 시작하세요.
...
- [Skill 워크플로](SKILL.md)
- [중문 런타임 어댑터](references/xxd-panel-040-prompt.zh-CN.md)
- [영문 런타임 어댑터](references/xxd-panel-040-prompt.en.md)
...
**먼저 사진을 읽고, 작은 인물이 그 이야기를 말하게 하세요.**
